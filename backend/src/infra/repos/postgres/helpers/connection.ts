...
export class PgConnection implements DbTransaction {
  ...
  getCustomRepository<Entity> (entity: ObjectType<Entity>): Entity {
    if (this.connection === undefined) throw new ConnectionNotFoundError()
    if (this.query !== undefined) return this.query.manager.getCustomRepository(entity)
    return getCustomRepository(entity)
  }
}
